...
$challanFont = trim((string)($company['challan_font_family'] ?? 'Arial, Helvetica, sans-serif'));
$allowedChallanFonts = [
...
if (!in_array($challanFont, $allowedChallanFonts, true)) {
    $challanFont = 'Arial, Helvetica, sans-serif';
}
$challanFontSize = (int)($company['challan_font_size'] ?? 9);
if ($challanFontSize < 7 || $challanFontSize > 12) {
    $challanFontSize = 9;
}
$items = $items ?? [];
...
{
    return number_format(max(0, (int)round((float)$value)), 0, '.', '');
}
  ...
  <div class="challan-inner">
    <div class="challan-header">
      <div class="challan-brand">
        ...
        <?php else: ?>
          <div class="logo-fallback"><?=esc(strtoupper(substr($companyName, 0, 2)))?></div>
        <?php endif; ?>
    ...
    <table class="challan-table">
      <thead>
        <tr><th>#</th><th>Product</th><th>Variant / Size</th><th>Quantity</th><th>Unit</th></tr>
      </thead>
      <tbody>
    ...
    <div class="challan-bottom">
      <div class="remarks">
        <b>Purpose / Remarks</b>
        <div><?=nl2br(esc($transaction['remarks'] ?: '—'))?></div>
      </div>
...
html,body{margin:0;padding:0;background:#e9edf3;color:#18202a;font-family:<?=esc($challanFont)?>}
body{font-size:<?=$challanFontSize?>px;line-height:1.25}
...
.sheet{width:210mm;height:297mm;margin:18px auto;background:#fff;padding:0;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 8px 35px rgba(15,23,42,.14)}
.challan-copy{height:135mm;flex:0 0 135mm;padding:2.5mm 0;border:1px solid #bfc6d0;overflow:hidden;break-inside:avoid;page-break-inside:avoid;position:relative}
.challan-copy:first-child{border-bottom:0}.challan-copy:last-child{border-top:0}
.challan-inner{width:100%;transform-origin:top left}
.challan-header{display:flex;justify-content:space-between;align-items:flex-start;gap:8px;border-bottom:1.5px solid #18202a;padding:0 3.5mm 4px}
...
.challan-title{text-align:center;font-size:13px;font-weight:800;letter-spacing:1.5px;margin:4px 3.5mm}
.challan-meta{display:grid;grid-template-columns:repeat(4,1fr);border:1px solid #c8ced6;margin:0 3.5mm}.challan-meta div{padding:4px 5px;border-right:1px solid #c8ced6;min-width:0}.challan-meta div:last-child{border-right:0}.challan-meta b,.party b{display:block;font-size:6.5px;text-transform:uppercase;color:#697382;margin-bottom:1px}.challan-meta span{font-weight:700;font-size:7.5px;overflow-wrap:anywhere}
.party{margin:3px 3.5mm 0;padding:3px 5px;border:1px solid #c8ced6;display:flex;gap:7px;align-items:baseline}.party b{margin:0;white-space:nowrap}.party span{font-size:8px;font-weight:700}
.challan-table{width:calc(100% - 7mm);margin:3px 3.5mm 0;border-collapse:collapse;table-layout:fixed}.challan-table th{background:#f0f2f5;text-align:left;font-size:6.5px;text-transform:uppercase;letter-spacing:.3px}.challan-table th,.challan-table td{border:1px solid #c8ced6;padding:3px 4px;vertical-align:middle}.challan-table th:nth-child(1){width:24px}.challan-table th:nth-child(2){width:28%}.challan-table th:nth-child(3){width:35%}.challan-table th:nth-child(4){width:16%}.challan-table th:nth-child(5){width:9%}.challan-table td:first-child{text-align:center}.challan-table .qty{font-weight:800;text-align:right}.challan-table small{display:block;color:#697382;font-size:6.5px;margin-top:1px}
.challan-bottom{display:grid;grid-template-columns:1fr 1fr;gap:7px;margin:3px 3.5mm 0}.remarks{border:1px solid #c8ced6;padding:4px;min-height:30px;font-size:7px}.remarks b{font-size:7px}.remarks div{margin-top:2px}.signatures{display:grid;grid-template-columns:1fr 1fr;gap:10px;text-align:center;color:#697382;font-size:6.5px}.signatures i{display:block;border-bottom:1px solid #555;margin:16px 0 2px}.signatures .sig-title{font-size:7px}.signatures small{font-size:6.5px}
.cut-line{height:7mm;flex:0 0 7mm;display:flex;align-items:center;justify-content:center;position:relative;color:#667080;font-size:7px;letter-spacing:1.8px}.cut-line:before{content:'';position:absolute;left:0;right:0;border-top:1px dashed #7a8493}.cut-line span{position:relative;background:#fff;padding:0 10px;font-weight:700}
...
@media print{
  ...
  html,body{width:210mm!important;height:297mm!important;min-width:210mm!important;margin:0!important;padding:0!important;background:#fff!important;overflow:hidden!important}
  .screen-bar{display:none!important}
  .sheet{position:absolute!important;left:0!important;top:0!important;width:210mm!important;height:297mm!important;min-width:210mm!important;min-height:297mm!important;margin:0!important;padding:0!important;box-shadow:none!important;overflow:hidden!important;transform:none!important}
  .challan-copy{height:135mm!important;flex:0 0 135mm!important;margin:0!important;overflow:hidden!important}
  .cut-line{height:7mm!important;flex:0 0 7mm!important;margin:0!important}
}
</style>
</head>
<body>
...
<script>
(function(){
  function fitChallans(){
    document.querySelectorAll('.challan-copy').forEach(function(copy){
      var inner=copy.querySelector('.challan-inner');
      if(!inner)return;
      inner.style.transform='scale(1)';
      var available=copy.clientHeight-8;
      var needed=inner.scrollHeight;
      if(needed>available){
        inner.style.transform='scale('+Math.max(0.55,available/needed)+')';
      }
    });
  }
  window.addEventListener('load',fitChallans);
  window.addEventListener('beforeprint',fitChallans);
})();
</script>
</body>
</html>